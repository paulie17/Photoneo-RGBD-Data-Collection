...
CalibrationSettings CalibrationSettings::load(const std::string& settingsPath) {
    CalibrationSettings settings;
    std::cerr << "settingsPath " << settingsPath << "\n";
    std::ifstream focalLengthFile(path::join(settingsPath, "FocalLength.txt"));
    ...
    std::ifstream pixelSizeFile(path::join(settingsPath, "PixelSize.txt"));
    ...
    settings.markersPositions = path::join(settingsPath, "MarkersPositions.txt");
...
pho::api::AdditionalCameraCalibration loadCalibration(
    ...
    const auto calibrationPath = path::calibrationFile(projectFolderPath);

    if (!path::readable(calibrationPath))
...
void calibrateAndSave(
    ...
    auto settings = CalibrationSettings::load(path::settingsFolder(projectFolderPath));

    try {
        auto calibration = calibrate(device, frames, images, settings);
        std::cout << "External camera calibration was successfull!" << std::endl;
        calibration.SaveToFile(path::join(projectFolderPath, "calibration.txt"));
    }
...
void calibrateFromFiles(
    ...
    if (frameNames.size() < 5)
        throw std::runtime_error("Need at least 5 frame / image pairs");
    ...
    if (!prawNames.size())
        prawNames.push_back(path::join(path::dataFolder(projectFolderPath), "1.praw"));

    // Attach praw file as FileCamera
    AttachedFileCamera fileCamera{factory, prawNames};
    auto device = fileCamera.connect();

    calibrateAndSave(device, projectFolderPath, frames, images);
...
void calibrateInteractive(
        pho::api::PhoXiFactory& factory, const std::string& projectFolderPath) {
    ...
    while (shouldContinue()) {
        ...
        frames.push_back(triggerScanAndGetFrame(device)->Texture);
